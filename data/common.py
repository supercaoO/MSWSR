...
def np2Tensor_mhr(l, rgb_range):
    ...
    return _np2Tensor(l[0]), [_np2Tensor(_l) for _l in l[1]]


def np2Tensor_mlrmhr(l, rgb_range):
    def _np2Tensor(img):
        # if img.shape[2] == 3: # for opencv imread
        #     img = img[:, :, [2, 1, 0]]
        np_transpose = np.ascontiguousarray(img.transpose((2, 0, 1)))
    ...
    return [_np2Tensor(_l) for _l in l[0]], [_np2Tensor(_l) for _l in l[1]]
...
def get_patch_mhr(img_in, img_tar, patch_size, scale):
    ih, iw = img_in.shape[:2]
    ...
    for i in range(len(scale)):
        oh, ow = img_tar[i].shape[:2]
        if ih == oh:
            tp = ip
            tx, ty = ix, iy
        else:
            tp = ip * scale[i]
            tx, ty = scale[i] * ix, scale[i] * iy

        img_tars.append(img_tar[i][ty:ty + tp, tx:tx + tp, :])

    return img_in, img_tars


def get_patch_mlrmhr(img_ins, img_tars, patch_size, scale):
    ins = []
    tars = []
...
def augment_mhr(img_list, hflip=True, rot=True):
    # horizontal flip OR rotate
    hflip = hflip and random.random() < 0.5
    vflip = rot and random.random() < 0.5
    rot90 = rot and random.random() < 0.5
    ...
    return _augment(img_list[0]), [_augment(img) for img in img_list[1]]


def augment_mlrmhr(img_list, hflip=True, rot=True):
    # horizontal flip OR rotate
    hflip = hflip and random.random() < 0.5
    vflip = rot and random.random() < 0.5
    rot90 = rot and random.random() < 0.5
    ...
    return [_augment(img) for img in img_list[0]], [_augment(img) for img in img_list[1]]
