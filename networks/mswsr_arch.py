...
class MSWSR(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, num_simdb, max_upscale_factor, ms, act_type='prelu',
                 norm_type=None):
        super(MSWSR, self).__init__()

        padding = 2
        self.num_features = num_features
        self.max_upscale_factor = max_upscale_factor
        self.max_num_steps = int(np.log2(self.max_upscale_factor) + 1)
        self.ms = ms
        ...
        for step in range(2, self.max_num_steps):
    ...
    def forward(self, x, scale):
        ...
        # sr = self.ifm((Yl, Yh)) / 2**(5 - self.num_steps)
        sr = [self.ifm((Yl, Yh)) / 2 ** (self.max_num_steps - num_steps)]
        if self.ms:
            for i in range(1, num_steps - 1):
                sr.insert(0, self.ifm((Yl, Yh[i:])) / 2 ** (self.max_num_steps - num_steps + i))
