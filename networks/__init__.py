...
def define_net(opt):
    ...
    if which_model.find('MSWSR') >= 0:
        from .mswsr_arch import MSWSR
        net = MSWSR(in_channels=opt['in_channels'], out_channels=opt['out_channels'], num_features=opt['num_features'],
                    num_simdb=opt['num_groups'], max_upscale_factor=opt['max_scale'],
                    ms=opt['ms_loss'] if 'ms_loss' in opt.keys() else False)
