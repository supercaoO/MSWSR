...
def parse(opt_path):
    ...
    max_scale = opt['max_scale'] if 'max_scale' in opt.keys() else None
    if 'max_scale' in opt.keys() and opt['is_train']:
        opt['scale'] = max_scale
    ...
    for phase, dataset in opt['datasets'].items():
        ...
        if phase == 'train':
            if dataset['mode'] == 'SLRMHR':
                opt['ms_loss'] = True
                dataset['scale'] = [2 ** (i + 1) for i in range(int(np.log2(opt['scale'])))]
            else:
                dataset['scale'] = opt['scale']
        elif phase == 'val':
            if dataset['mode'] == 'MLRMHR':
                opt['ms_val'] = True
                dataset['scale'] = [2 ** (i + 1) for i in range(int(np.log2(opt['scale'])))]
            else:
                dataset['scale'] = opt['scale']
    ...
    opt['networks']['max_scale'] = max_scale
    opt['networks']['scale'] = opt['scale']
    opt['networks']['ms_loss'] = opt['ms_loss']
