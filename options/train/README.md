...
Let us take [`train_MSWSR_x248.json`](./train_WSR_x248.json) as an example. 

**Note**: Before you run `python train.py -opt options/train/*.json`, please carefully check options: `"max_scale"`, `"dataroot_HR"`, `"dataroot_LR"`, `"networks"` and `"pretrained_path"` (if `"pretrain"` option is set to `"resume"` or `"finetune"`).
...
```c++
{
    ...
    "datasets": { 
        // train dataset
        "train": { 
            "mode": "SLRMHR", // dataset mode: "SLRMHR" | "MLRMHR" | "LRHR" | "LR", "SLRMHR" is required during training
            "dataroot_HR": "./dataset/MSAugment842/DIV2K_train_HR_aug/", // HR dataset root
            "dataroot_LR": "./dataset/MSAugment842/DIV2K_train_LR_aug/x8", // LR dataset root
        ...
        // validation dataset
        "val": {
            "mode": "MLRMHR",  // dataset mode: "SLRMHR" | "MLRMHR" | "LRHR" | "LR", "MLRMHR" is required during validating
    ...
    "solver": {
        ...
        "pretrained_path": "./experiments/MSWSR_in3f32_x4/epochs/last_ckp.pth",
        // weights for multiple wavelet coefficients (required by our MSWSR, and number = log2(max_scale) + 1)
