...
def main():
    ...
    scale = [2 ** (i + 1) for i in range(int(np.log2(opt['scale'])))] if 'ms_val' in opt.keys() and opt['ms_val'] else [opt['scale']]
    ...
    print("Method: %s || Scale: %s || Epoch Range: (%d ~ %d)" % (model_name, scale, start_epoch, NUM_EPOCH))
    ...
    for epoch in range(start_epoch, NUM_EPOCH + 1):
        ...
        psnr_list = [[] for _ in range(len(scale))]
        ssim_list = [[] for _ in range(len(scale))]
        val_loss_list = []

        for iter, batch in enumerate(val_loader):
            solver.feed_data(batch)
            iter_loss = solver.test(scale)
            ...
            if opt["save_image"]:
                solver.save_current_visual(epoch, iter, scale)

        solver_log['records']['val_loss'].append(sum(val_loss_list) / len(val_loss_list))
        for i in range(len(scale)):
            solver_log['records']['psnrx' + str(scale[i])].append(sum(psnr_list[i]) / len(psnr_list[i]))
            solver_log['records']['ssimx' + str(scale[i])].append(sum(ssim_list[i]) / len(ssim_list[i]))

        # record the best epoch
        epoch_is_best = False
        if solver_log['best_pred'] < (sum(psnr_list[-1]) / len(psnr_list[-1])):
            solver_log['best_pred'] = (sum(psnr_list[-1]) / len(psnr_list[-1]))
            epoch_is_best = True
